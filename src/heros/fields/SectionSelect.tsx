...
/**
 * Custom admin field: a dropdown of every layout block that has an Anchor ID.
 * Reads the live form state (the document's `layout` blocks) so editors pick an
 * existing section instead of typing a raw "#anchor". Stores the value as "#anchor".
 */
export const SectionSelect: React.FC<SectionSelectProps> = ({ path, field, readOnly }) => {
  ...
  const label = typeof field?.label === 'string' ? field.label : 'Page section'
